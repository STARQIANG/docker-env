...
if [[ -n "$ELASTIC_HOST_COMMAND" ]]; then
    HOST_VALUE=$(eval "$ELASTIC_HOST_COMMAND")
    es_opt="-Enetwork.publish_host=${HOST_VALUE}"
    es_opts+=("${es_opt}")
fi

if [[ -n "$ELASTIC_NODE_NAME_COMMAND" ]]; then
    ELASTIC_NODE_NAME_VALUE=$(eval "$ELASTIC_NODE_NAME_COMMAND")
    es_opt="-Enode.name=${ELASTIC_NODE_NAME_VALUE}"
    es_opts+=("${es_opt}")
fi
